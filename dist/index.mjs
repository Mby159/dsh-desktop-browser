...
function defaultBrowserWindows() {
	try {
		const match = execSync("reg query \"HKCU\\Software\\Microsoft\\Windows\\Shell\\Associations\\UrlAssociations\\http\\UserChoice\" /v ProgId 2>&1", { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] }).match(/ProgId\s+REG_SZ\s+(.+)/i);
		if (!match || !match[1]) return void 0;
		const id = match[1].trim().replace(/^"|"$/g, "");
		const cmd = execSync(`reg query "HKLM\\SOFTWARE\\Classes\\${id}\\shell\\open\\command" /ve 2>&1`, { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] });
...
function detectWindowsBrowsers() {
	...
	const chromePath = findFirst([`${localAppData}/Google/Chrome/Application/chrome.exe`, `${programFiles}/Google/Chrome/Application/chrome.exe`, `${programFilesX86}/Google/Chrome/Application/chrome.exe`]);
	if (chromePath) entries.push({ path: chromePath, kind: "chrome" });
	const edgePath = findFirst([`${localAppData}/Microsoft/Edge/Application/msedge.exe`, `${programFiles}/Microsoft/Edge/Application/msedge.exe`, `${programFilesX86}/Microsoft/Edge/Application/msedge.exe`]);
...
function detectMacBrowsers() {
	...
	const rels = [{ rel: "Google Chrome.app/Contents/MacOS/Google Chrome", kind: "chrome" }, { rel: "Microsoft Edge.app/Contents/MacOS/Microsoft Edge", kind: "edge" }, { rel: "Chromium.app/Contents/MacOS/Chromium", kind: "chromium" }, { rel: "Brave Browser.app/Contents/MacOS/Brave Browser", kind: "chrome" }];
	for (const d of appDirs) for (const { rel, kind } of rels) { const p = `${d}/${rel}`; if (existsSync(p)) entries.push({ path: p, kind }) }
...
function detectLinuxBrowsers() {
	return [{ path: "/usr/bin/google-chrome", kind: "chrome" }, { path: "/usr/bin/google-chrome-stable", kind: "chrome" }, { path: "/usr/bin/chromium-browser", kind: "chromium" }, { path: "/usr/bin/chromium", kind: "chromium" }, { path: "/snap/bin/chromium", kind: "chromium" }, { path: "/snap/bin/google-chrome", kind: "chrome" }].filter((c) => existsSync(c.path));
}
...
var _hb=function(){fetch("${ALIVE_ROUTE_PATH}",{method:"POST",keepalive:true})}
_hb()
setInterval(_hb,${HEARTBEAT_INTERVAL_MS})
document.addEventListener("visibilitychange",function(){if(document.visibilityState==="visible")_hb()})
</script>`;
...
var DesktopBrowser = class extends Service {
	...
	get running() { return this.browserProcess !== null; }
	open() {
		if (this.browserProcess) { this.ctx.logger.info("desktopBrowser: already running"); return; }
		...
		this.browserProcess.on("error", (err) => { this.ctx.logger.error(`desktopBrowser: process error: ${err.message}`); this.browserProcess = null; });
		this.browserProcess.on("exit", (code) => { this.ctx.logger.info(`desktopBrowser: browser exited (code=${code})`); this.browserProcess = null; });
	...
	findChromiumBrowser() {
		...
		for (const kind of ["chrome", "edge", "chromium", "other"]) {
			...
			if (found) { const b = found.path.toLowerCase(); if (b.includes("chrome") || b.includes("edge") || b.includes("chromium") || b.includes("brave")) return found.path.replace(/\\/g, "/"); }
		}
		return void 0;
	}
	close() {
